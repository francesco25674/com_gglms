...
class sendmail {
	...
	private function header($cc, $bcc) {
		if (!isset($this->_from['mail']))
			throw new Exception('From not set.', E_USER_ERROR);
		// from
	...
	public function send() {
		try {
			if (empty($this->_to))
				throw new Exception('No receivers specified', E_USER_ERROR);
			if (empty($this->_body)) 
				throw new Exception('No body specified', E_USER_ERROR);
