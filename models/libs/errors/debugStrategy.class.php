...
require_once('libs/phpinfo.lib.php');
require_once('libs/clientinfo.lib.php');
...
/**
  ...
  * @todo Possibilita' di passare come opzioni al costruttore 'souce', 'source_line', 'trace' come parametri 
  * per tutte le classi debugStrategy 
  * @author jbrond
  */
abstract class debugStrategy {

	/**
	 * Runtime configuration options of the debug strategy.
	 *
	 * @var array
	 */
	protected $_options;
	...
	public function __construct($options=null) {
		...
		if (!isset($options['debug_level'])) {
			$this->_options['debug_level'] = DEBUG_DEFAULT_LEVEL;
		}
	}
	...
	public function checkpoint($msg='') {
		if ($this->_options['debug_level'] & DEBUG_LOG) {
	...
	public function memory($msg='') {
		...
		if (preg_match('/(\d+)(\w+)/', $limit, $matches)) { 
			if ('M' === $matches[2]) $limit = $matches[1] * 104857;
			elseif ('K' === $matches[2]) $limit = $matches[1] * 1024;
			$perc = sprintf('%.2f', ($usage*100)/$limit);
		} else {
			$perc = '?';
		}
		$this->msg('Memory usage '.$msg.' '.$usage.' ['.$perc.'%]', DEBUG_INFO);
		unset($limit);
		unset($usage);
		unset($perc);
	}
	...
	public function exception(Exception $e, $trace=DEBUG_DEFAULT_TRACE, $source=DEBUG_DEFAULT_SOURCE, $lines=DEBUG_DEFAULT_SOURCE_LINES) {
		if (error_reporting() & $e->getCode()) {
			...
			if ($source && method_exists($e,'get_source_str')) {
				$msg .= PHP_EOL.$e->get_source_str($lines);
			}
