...
class debugFirebug extends debugStrategy {
    ...
    * - show_line (bool): include File and Line information in message (Default true).
	...
	public function __construct($options=null) {
		...
		$max_depth = isset($options['max_depth']) ? filter_var($options['max_depth'], FILTER_VALIDATE_INT, array('options'=>array('default'=>10, 'min_range'=>0))) : 10;
		//$max_depth = (isset($options['max_depth']) && $options['max_depth']>0) ? $options['max_depth'] : 10;
		$FBoptions = array(
			'maxObjectDepth' => $max_depth,
			'maxArrayDepth' => $max_depth,
			'maxDepth' => $max_depth,
			'useNativeJsonEncode' => true,
			'includeLineNumbers' => isset($options['show_line']) ? $options['show_line'] : true
		);
		FB::setEnabled(true);
	...
	public function vardump($var, $name=null) {
		if ($this->_options['debug_level'] & DEBUG_DUMP) {
			FB::send($var, $name);
		}
	}
	...
	public function exception(Exception $e, $trace=DEBUG_DEFAULT_TRACE, $source=DEBUG_DEFAULT_SOURCE, $lines=DEBUG_DEFAULT_SOURCE_LINES) {
		if (error_reporting() & $e->getCode()) { 
			...
			if ($source && method_exists($e,'get_source')) {
