...
class debug {
     ...
     *
     * @param string $strategy Nome della strategia (es. 'firebug', 'log').
    ...
    public static function startNested($strategy, $options = null, $name = null) {
        ...
        if (!isset(self::$_strategies[$strategy_name])) {
            ...
            self::$_strategies[$strategy_name] = ($strategy == 'Log') ? new debugLog($options) : new debugFirebug($options);
        }
        self::$_strategies_stack[++self::$_stack_head] = array($strategy_name);
        print_r(self::$_strategies_stack);
        return 1;
    }
     ...
     * Si possono avere piu' strategie in parallelo ognuna con opzioni diverse.
     *
     * @param string $strategy Nome della strategia (es. 'firebug', 'log').
     * @param array $options Array di opzioni passato al costruttore della strategia di debug.
     * @param string $name Nome delle strategia, se non specificato verra' usato il valore di $strategy. Se usato il nome della strategia sara': $strategy_$name.
    ...
    public static function start($strategy, $options = null, $name = null) {
        ...
        if (!isset(self::$_strategies[$strategy_name])) {
            ...
            if ($strategy == 'Log')
                self::$_strategies[$strategy_name] = new debugLog ($options);
            else
                self::$_strategies[$strategy_name] = new debugFirebug($options);
        }
     ...
     *
     * @param string $strategy Nome della strategia (es. 'firebug' o 'log').
    ...
    public static function end($strategy, $name = null) {
        ...
        if (self::$_stack_head) {
            ...
            if (count(self::$_strategies_stack[self::$_stack_head])) {
                unset(self::$_strategies_stack[self::$_stack_head--]);
            }
            //sort(self::$_strategies_stack[$index]);
        }
        return $return;
    }
     ...
     */
    public static function exception(Exception $e, $trace = DEBUG_DEFAULT_TRACE, $source = DEBUG_DEFAULT_SOURCE, $lines = DEBUG_DEFAULT_SOURCE_LINES) {
        foreach (self::active() as $strategy) {
    ...
    /**
     * Lancia le funzioni di debug per ogni strategia attiva.
     * Le funzioni a disposizione sono @see debugObject::msg(), @see debugObject::vardump(), @see debugObject::checkpoint(),
     * @see debugObject::chrono_start(), @see debugObject::chrono_stop(), @see debugObject::exception(), @see debugObject::memory().
     *
     * @param string $func
     * @param array $args
     */
    /* public static function __callStatic($func, $args) {
      foreach (self::active() as $strategy) {
      call_user_func_array(array(self::$_strategies[$strategy], $func), $args);
      }
      } */
...
function PokemonExceptionHandler(Exception $e) {
    debug::exception($e); //msg('Uncaught exception: '.(string)$e);
}
