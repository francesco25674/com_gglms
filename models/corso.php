...
//TODO Modifica non testata, assicurarsi che funziona!!
        ...
        if ($e['tipologia'] == 'exam') {
            ...
            $query = 'SELECT id_quiz FROM #__gg_final_test WHERE id_utente=' . $user_id . ' AND attempt_time IS NULL  AND id_corso= '.$e['id_corso'].' LIMIT 1';
            debug::msg($query);
            $this->_db->setQuery($query);
            if (false === ($results = $this->_db->loadAssoc()))
                throw new RuntimeException($this->_db->getErrorMsg(), E_USER_ERROR);
...
//Se si svilupperà anche la la parte di quiz la verifica di stato potrà avvenire solo sulla tabella track
        ...
        FROM
            #__gg_corsi AS c
        INNER JOIN #__gg_corsi_versione AS cv ON cv.id_corso = c.id
        INNER JOIN #__gg_moduli AS m ON m.id_corso = cv.id
                ...
                case 'exam':
                    $query = '
                            ...
                            FROM
                                #__quiz_r_student_quiz q
                            WHERE
                                q.c_quiz_id IN (SELECT id_quiz FROM #__gg_final_test WHERE id_utente=' . $user_id . ' AND attempt_time IS NOT NULL AND id_corso= '.$e['idcorso'].') AND
                                q.c_passed = 1 AND
    ...
    private function getElementi($id_modulo) {
        try {
            ...
            $query = '
                ...
                LEFT JOIN #__gg_moduli AS m on e.id_modulo = m.id
